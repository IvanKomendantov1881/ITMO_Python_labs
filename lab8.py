...
# cv2.imwrite('variant-6_resized.png', img)
#
# cv2.imshow('img', img)
# cv2.waitKey(0)
# cv2.destroyWindow()
...
def video_processing():
    ...
    while True:
        ...
        if len(contours) > 0:
            ...
            if center_x > frame_center and flag == 0:
                right += 1
                flag = 1

            elif center_x < frame_center and flag == 1:
                left += 1
                flag = 0
            cv2.putText(frame, f'Left: {left}', (10, 40), cv2.FONT_HERSHEY_SIMPLEX, 1, (255, 0, 0), 2)

            cv2.putText(frame, f'Right: {right}', (10, 80),cv2.FONT_HERSHEY_SIMPLEX, 1, (0, 0, 255), 2)
